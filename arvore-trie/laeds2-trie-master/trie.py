...
class Trie:
	...
	def preditor_palavras(self, no_atual:"NoTrie", palavra:str = None, palavra_list:list = None):
		...
		filhos_no_atual = no_atual.nos_filhos()
		#print(filhos_no_atual)
		for letra in filhos_no_atual:
			if no_atual.existe_letra(letra):            
				no_atual = no_atual.obtem_no_filho(letra)
				self.preditor_palavras(no_atual, palavra + letra, palavra_list)           
