...
class TestNode(unittest.TestCase):
    ...
    def test_max_depth(self):
        ...
        for i in range(0, 4):
            #print("Max distancia = " + f"{self.arr_tests[i].position_node(8)}")
    ...
    def test_position_node(self):
        ...
        #arr_expected = [123, None, 6, 14]

        for i in range(3,7):
            #print("Posicao do no 8 = " + f"{self.arr_tests[i].position_node(8)}")
            self.assertEqual(self.arr_tests[i].position_node(8),arr_expected[i-3],f"Erro na árvore de teste posição {i-3}")
            #self.assertEqual(self.arr_tests[i].position_node(9),arr_expected[i-3],f"Erro na árvore de teste posição {i-3}")
